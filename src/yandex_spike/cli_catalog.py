...
CLI_COMMANDS: tuple[CliCommand, ...] = (
    ...
    CliCommand(
        "menu",
        "TUI-меню (Textual)",
        "Левый бар команд, прогресс, статусы auth/переноса.",
    ),
    CliCommand(
        "menu-classic",
        "Классическое меню",
        "Простой выбор по номеру без Textual (fallback).",
        in_menu=False,
    ),
    CliCommand(
        "auth-implicit",
...
def format_help_text() -> str:
    lines = [
        ...
        "",
        "Запуск:",
        "  uv run yandex-spike              -> TUI-меню Textual (в TTY)",
        "  uv run yandex-spike help         -> эта справка",
        "  uv run yandex-spike menu         -> TUI явно",
        "  uv run yandex-spike menu-classic -> простое меню без Textual",
